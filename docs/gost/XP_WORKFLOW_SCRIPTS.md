...
- `verify-angle-trace-xp-codegen.ps1` — ANGLE local-static codegen gate for the XP x86 `/Zc:threadSafeInit-` remediation. `Focused` mode additionally verifies the exact `clang-cl` commands for `Display.cpp` and `formatutils.cpp`; `FullBuild` mode operates on the objects emitted by the full browser build. Both modes require the original function-local statics to remain in source, reject `_Init_thread_header`, `_Init_thread_footer` and `_Init_thread_epoch` evidence from both compiled objects, and preserve matching `libGLESv2` evidence.
