...
- `verify-angle-trace-xp-codegen.ps1` — focused ANGLE local-static codegen gate for the XP x86 `/Zc:threadSafeInit-` experiment. It requires the original function-local statics to remain present at the two physically reproduced owners (`Display.cpp` trace category and `formatutils.cpp::GetAllSizedInternalFormats`), verifies both translation units compile with `MOZ_XP_COMPAT`, optimization and `/Zc:threadSafeInit-`, rejects `_Init_thread_header`, `_Init_thread_footer` and `_Init_thread_epoch` evidence from both compiled objects, and preserves the matching `libGLESv2.pdb`.
