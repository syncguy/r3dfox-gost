...
The first runtime trust smoke is also positive on Windows 7 x64 for the Russian RSA/NSS path. The user launched the trust-package lineage with `R3DFOX_GOST_HOSTS=lk.zakupki.gov.ru` and navigated to `https://zakupki.gov.ru/epz/main/public/home.html`. `about:policies` shows the `Certificates` policy active with `ImportEnterpriseRoots=true` and both installed root filenames; the main EIS page renders successfully; and Firefox certificate details show `*.zakupki.gov.ru -> Russian Trusted Sub CA -> Russian Trusted Root CA`. Because the tested main host is `zakupki.gov.ru`, not the configured allowlist token `lk.zakupki.gov.ru`, this result is ordinary Firefox/NSS trust evidence rather than an MSSPI GOST handshake result. The package does not explicitly bundle the Sub CA, so the successful chain confirms that an explicit intermediate bundle is not required for this observed RSA path.

Reference package hashes computed directly from `r3dfox-v153.0.3.win64.zip` inside artifact `9838528394` are `r3dfox.exe` SHA-256 `a439940ba92e70f14b1997f7d82e5beceb2ef6aa4517c21ca9001311cfa13aa7` and `xul.dll` SHA-256 `8ebda2b3337e2fe9c88a7191885e509d55fb1fa2cbb3c5ca54e1df4be8b323d6`. The current user-supplied runtime evidence does not yet include local hash output matching those values, and the profile path `C:\Temp\r3dfox\profile` was not independently demonstrated to be newly clean.

Current trust status: **package/staging closure is GREEN; Win7 x64 Russian RSA/NSS runtime smoke is GREEN; full clean-profile exact-artifact runtime acceptance remains OPEN.** Remaining closure items are local binary-hash equality, explicit clean-profile confirmation, direct proof that `security.enterprise_roots.enabled` is effectively true/locked under policy, a Windows-store-only trust case that is not conflated with the bundled RSA anchor, and runtime coverage of the bundled GOST root / relevant GOST PKI path.
