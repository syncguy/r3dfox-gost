...
This closes the full-build/static acceptance boundary for the ANGLE remediation. It does **not** prove physical XP WebGL behavior. The next boundary is physical XP validation of the exact resulting artifact: verify binary identity, create a WebGL context and exercise rendering; predecessor DLL RVAs are historical fault identities, not transferable acceptance breakpoints for this newly linked DLL.
