...
The remaining blocker is stability, not initial WebGL bring-up. The GPU child still terminates intermittently after rendering. The current DrWatson capture records `0x80000007 / STATUS_WAKE_SYSTEM_DEBUGGER` rather than a new `0xC0000005` fault location, so it does not localize the new failure owner. Exact local-binary-to-artifact correlation for run `35980235042` is also still pending. Do not attribute this intermittent failure to ANGLE/D3D9/mozglue until a matching-symbol exception capture establishes the actual boundary.
