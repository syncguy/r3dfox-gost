...
A separate WinDbg capture on the same exact package has now localized a first-chance `0xC0000005` in the GPU child (PID `0xAAC` / 2732, parent PID 2384) at `xul.dll+0x0090DED4`. Matching `xul.pdb` resolves the path through `nsThreadManager::get()` -> `nsThread::MaybeRemoveFromThreadList()` -> `nsThread::~nsThread()` -> `nsThreadManager::ReleaseThread`. The faulting thread has xul `_tls_index=5` but its TEB TLS slot 5 is null, while the same slot is non-null on 24 other threads. The correct dump proves the thread started inside xul's Rust `std::thread` entry path, so this is not a thread that merely predates xul loading.

The GPU-child stack is in `nss3.dll` `DLL_THREAD_DETACH` cleanup for an NSPR-attached foreign thread. The exact xul PE contains the intended YY-Thunks `DllMainCRTStartupForYY_Thunks` entry/TLS contract. Under the XP YY dynamic-TLS path, xul thread detach clears its emulated TLS slot in `FreeTlsData()`; later nss3/NSPR detach cleanup invokes the xul-owned NSPR TPD destructor `nsThreadManager::ReleaseThread`, re-entering xul after that slot is null. This teardown-order incompatibility is the currently localized GPU-child blocker. It is separate from the still-unlocalized parent-process AV and does not reopen the predecessor DWrite-loader blocker.
