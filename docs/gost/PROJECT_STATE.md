...
8. **CryptoPro provider UI currently blocks the Mozilla Socket Thread.** The interactive insert-media operation occurs synchronously inside `msspi_connect()`. In the confirmed capture the socket-thread call is held for about 14.1 seconds until provider Cancel on the first attempt and about 27.0 seconds until media insertion on the successful retry. This is distinct from the Firefox-picker busy-loop defect: the provider UI blocks rather than spins the Socket Thread.
...
2. Integrate the picker wait with Firefox/Necko client-auth lifecycle so an active user decision does not busy-poll and is not treated as an ordinary unfinished TLS handshake subject to the 30-second timeout.
3. Investigate the synchronous CryptoPro/SSPI provider-interaction path. Determine whether private-key acquisition/interactive provider work can be moved off or otherwise prevented from monopolizing the global Socket Thread while preserving the same Schannel/MSSPI security context. Do not solve this by speculative pre-enumeration that triggers provider UI.
4. Re-run Treasury login after the picker lifecycle fix and prove one user selection safely resumes all compatible sockets without queued dialogs, negative cache poisoning or the earlier application 500 symptom.
...
- Current Firefox-facing mTLS is demonstrably functional but not production-ready because picker timeout/busy-wait, stale callbacks, concurrent picker races, synchronous provider UI on the Socket Thread and final server-verification hardening remain open.
