...
10. **BCrypt remains a separate compatibility boundary.** `bcrypt.dll` is still directly imported by `xul.dll` and `mozglue.dll`. Continue the source-available/reproducible minimal-BCrypt-facade investigation separately from the general YY decision.
