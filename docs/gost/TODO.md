...
The source-built One-Core bcrypt dependency closure is now also physically proven at focused runtime scale: source-under-test `fdd4d4dac5a7d9611ec71975ae800437f45c47dd`, run `33493625367`, job `99810642354`, runtime artifact `9794971087`. On physical XP both the exact-local dynamic consumer and ordinary linked consumer load the local source-built `bcrypt.dll`, with its source-built `mbedtls.dll` dependency, and pass export, RNG and SHA-256 checks with exit code 0. The focused bcrypt research item is closed; the remaining work is transfer of this proven closure into the full browser build/package.
...
10. **Transfer the physically proven source-built BCrypt closure into the full XP x32 browser.** Stage the exact source-built `bcrypt.dll + mbedtls.dll` contract proven by run `33493625367` into the Firefox build/package, preserve pinned upstream source/provenance, audit both DLLs, require package survival, and then bind the resulting browser artifact to a physical-XP startup test. Do not reopen the focused bcrypt implementation question unless this exact integration shows new evidence.
