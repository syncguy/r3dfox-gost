...
This file is the persistent forward-looking backlog. Current synthesis is in `PROJECT_STATE.md`; exact runtime test sequencing/recovery is in `STAGE2_RUNTIME_TEST_PLAN.md`; the GIS GMP multi-host mTLS branch is in `STAGE2_GIS_GMP.md`; the WinRT source-removal alternative is in `WINRT_SOURCE_POC.md`; experiment evidence is in `TEST_LOG.md` and dated volumes.
...
3. **Win7 x32 later parent/browser crash — `xul.dll` WinRT delay-load blocker; compare two remedies.** On exact source `982d6529a707c6feecad97c725feed8a3cd21c81` / run `33141004769` / runtime artifact `9676549576`, the sandbox-off browser can browse real pages and then later crashes in the parent process. ProcMon plus first-chance WinDbg prove: `xul.dll` delay-loads `api-ms-win-core-winrt-l1-1-0.dll`, attempts `RoGetActivationFactory`, Win7 returns `ERROR_MOD_NOT_FOUND` (`0x7e`), and the MSVC delay-load helper raises `0xc06d007e`. Keep two independent implementation hypotheses until evidence selects one:
   - **A — narrow YY-Thunks WinRT coverage.** Identify the exact YY-Thunks 1.2.2 archive/member set for `RoGetActivationFactory` and the companion WinRT string APIs that `xul.dll` also delay-imports; extend the existing physically narrow provider and rebuild. Do not inject a fake API-set DLL and do not switch to broad complete-YY `kernel32.lib` interposition.
   - **B — source-level WinRT removal/fallback.** The isolated branch `agent/winrt-source-poc` removes/stubs nonessential WinRT consumers while preserving interfaces or legacy fallbacks: WinRT ToastNotification -> stub + XUL fallback, WinRT InputPane -> legacy no-op, Windows widget Rust -> no `windows`/`windows-core` WinRT dependency with empty history/unsupported permission monitoring, selected package/UI sources -> existing non-WinRT conditionals. See [`WINRT_SOURCE_POC.md`](./WINRT_SOURCE_POC.md) for the exact design and file inventory. Reviewed HEAD `f1c55320f77a9ff932fa3e6cca2bc2443c20ec45` is 8 commits ahead of main base `3ebfef1ddbb70b0d2b29f160dabcaa8fbef4fab5`, 0 behind. This PoC is **not validated**: run `33242986136`, job `99075394860`, source `f6e87ba87919f999d2a3ec6c2b9ba103fea1d99e`, and run `33243005271`, job `99075448278`, source `f1c55320f77a9ff932fa3e6cca2bc2443c20ec45`, both stopped at `Build export prerequisites` before the relevant target/full-build proof gates. Do not merge the branch on design plausibility alone.
   - For strategy B, required proof is: repair export build -> targeted WinRT-related object compilation -> object/reference scan -> full x86 Firefox/xul build/package -> final `xul.dll` ordinary/delay-import audit -> physical Win7 runtime through the previous later-crash window -> separate physical XP runtime. If it works, reduce it before integration: replace `__MINGW32__` spoofing with an explicit legacy/no-WinRT capability flag and restore any windows-rs/Win32 surface that need not be removed.
4. **Choose by exact evidence, not patch size.** Compare A and B on final import surface, Win7 runtime stability and functional regressions. Source removal may eliminate the dependency class at the feature boundary but has greater source divergence; narrow thunking preserves more Firefox functionality but can require additional precisely selected aliases as new WinRT imports are discovered. Neither is currently closed.
5. Re-run the resulting exact full x86 build on physical Win7 and XP after each compatibility fix, treating each newly observed loader/runtime failure as a separate evidence-backed blocker rather than preemptively rewriting every audit finding.
6. Keep GOST TLS-on-old-Windows as a later exact-build/runtime milestone independent from loader/startup compatibility. A browser that starts and browses ordinary pages still does not prove the MSSPI GOST path on XP/Win7.
