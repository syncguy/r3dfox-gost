...
3. **Win7 x32 later parent/browser crash — `xul.dll` WinRT delay-load blocker; source-level removal/fallback is now the primary direction.** On exact source `982d6529a707c6feecad97c725feed8a3cd21c81` / run `33141004769` / runtime artifact `9676549576`, the sandbox-off browser can browse real pages and then later crashes in the parent process. ProcMon plus first-chance WinDbg prove: `xul.dll` delay-loads `api-ms-win-core-winrt-l1-1-0.dll`, attempts `RoGetActivationFactory`, Win7 returns `ERROR_MOD_NOT_FOUND` (`0x7e`), and the MSVC delay-load helper raises `0xc06d007e`.
   - **Do not continue broadening the YY-Thunks WinRT provider as the primary fix.** The dedicated YY WinRT smoke started at `90067edba48fd4e8bb986ced02a47ae2189e9fb3` and reached `3ebfef1ddbb70b0d2b29f160dabcaa8fbef4fab5` after 26 additional commits and about 3 h 26 min. The workflow alone accumulated 345 changed lines of churn while the compatibility surface expanded through WinRT aliases, CRT/harness stubs, MASM and sequential system-library additions (`ADVAPI32`, `GDI32`, `USER32`, `VERSION`, `NTDLL`, `OLEAUT32`, `OLE32`). Final run `33186862417`, job `98901994671`, source `3ebfef1...`, run number `31`, still failed in the synthetic closing link on newly exposed `__imp__StrCmpLogicalW@8`. This path is **retired as unpromising for this WinRT blocker**. Do not respond to that failure by simply adding the next alias/stub/library.
   - This retirement is WinRT-specific. Keep YY-Thunks for small, stable, evidence-bounded Win32 compatibility gaps; the representative XP x86 YY coexistence runtime remains proven. A residual surgical YY shim may be reconsidered only if source-level WinRT removal later leaves one or a few unavoidable imports with narrow semantics.
   - **Primary active experiment — source-level WinRT removal/fallback.** The isolated branch `agent/winrt-source-poc` removes/stubs nonessential WinRT consumers while preserving interfaces or legacy fallbacks: WinRT ToastNotification -> stub + XUL fallback, WinRT InputPane -> legacy no-op, Windows widget Rust -> no `windows`/`windows-core` WinRT dependency with empty history/unsupported permission monitoring, selected package/UI sources -> existing non-WinRT conditionals. See [`WINRT_SOURCE_POC.md`](./WINRT_SOURCE_POC.md) for the exact design and file inventory. Reviewed HEAD `f1c55320f77a9ff932fa3e6cca2bc2443c20ec45` is 8 commits ahead of main base `3ebfef1ddbb70b0d2b29f160dabcaa8fbef4fab5`, 0 behind.
   - The source-removal PoC is **not yet validated**: run `33242986136`, job `99075394860`, source `f6e87ba87919f999d2a3ec6c2b9ba103fea1d99e`, and run `33243005271`, job `99075448278`, source `f1c55320f77a9ff932fa3e6cca2bc2443c20ec45`, both stopped at `Build export prerequisites` before the relevant target/full-build proof gates. Promoting this path means it is the next experiment, not that it is already a fix.
   - Required proof: repair export build -> targeted WinRT-related object compilation -> object/reference scan -> full x86 Firefox/xul build/package -> final `xul.dll` ordinary/delay-import audit -> physical Win7 runtime through the previous later-crash window -> separate physical XP runtime. If it works, reduce it before integration: replace `__MINGW32__` spoofing with an explicit legacy/no-WinRT capability flag and restore any windows-rs/Win32 surface that need not be removed.
