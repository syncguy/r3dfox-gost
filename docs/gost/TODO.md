...
The source-built One-Core bcrypt dependency is now physically proven in the selected single-DLL form: source-under-test `a30a701fcf50eb08b6ea7574cb7cc927f6eae014`, run `33513084915`, job `99873297193`, runtime artifact `9802703271`. On physical Windows XP 5.1.2600, the extracted bundle contains only one DLL, `bcrypt.dll` (`520704` bytes), and both the exact-local dynamic consumer and ordinary linked/IAT consumer load that local DLL and pass exports, RNG and SHA-256 with exit code 0. The runtime `mbedtls.dll` dependency is eliminated because the pinned mbedTLS C modules are compiled directly into `bcrypt.dll`. Focused bcrypt implementation research is closed; remaining bcrypt work is transfer of this exact single-DLL contract into the full browser build/package.
...
10. **Transfer the physically proven single-DLL BCrypt contract into the full XP x32 browser.** Reproduce the exact source/build composition proven by source `a30a701fcf50eb08b6ea7574cb7cc927f6eae014`, run `33513084915`, job `99873297193`; stage only the resulting `bcrypt.dll`, preserve pinned upstream provenance, audit the DLL, require package survival, and then bind the resulting browser artifact to a physical-XP startup test. Do not reopen the focused bcrypt implementation question unless this exact integration shows new evidence.
