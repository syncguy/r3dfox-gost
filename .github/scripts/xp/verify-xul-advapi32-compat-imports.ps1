...
$diag = Join-Path $env:GITHUB_WORKSPACE 'xp-x32-advapi32-etw-import'
...
if (-not (Test-Path $xul)) {
  ...
  throw "xul.dll missing for ADVAPI32 compatibility gate: $xul"
}
...
$apis = @('EventRegister','EventUnregister','EventWrite','EventWriteTransfer','RegGetValueW')
$mode = 'none'
$currentDll = ''
...
foreach ($line in $raw) {
  ...
  if ($mode -ne 'direct' -or $currentDll -notmatch '(?i)^ADVAPI32\.dll$') {
    continue
  }
...
$unique | Set-Content -Encoding utf8 (Join-Path $diag 'surviving-etw-imports.txt')
if ($unique.Count -gt 0) {
  ("result=FAIL|surviving=" + ($unique -join ',')) | Set-Content -Encoding utf8 (Join-Path $diag 'result.txt')
  throw ("xul.dll still imports ADVAPI32 compatibility family members: " + ($unique -join ', '))
}
